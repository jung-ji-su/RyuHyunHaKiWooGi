...
export function useFCM(currentUser) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const reg = await navigator.serviceWorker.ready; // 활성화 완료 대기
  ...
  // 버튼 클릭 시 호출 — 에러는 throw로 전달 (iOS에서 alert 차단되므로)
  const requestPermission = useCallback(async () => {
    ...
    // ① 권한 요청 (iOS: 유저 제스처 직후 첫 번째로 호출)
    const permission = await Notification.requestPermission();
    if (permission !== 'granted') throw new Error(`권한 거부: ${permission}`);

    // ② SW 등록 후 활성화(active) 완료까지 대기 — 이 단계가 빠지면 iOS에서 networkError
    await navigator.serviceWorker.register('/firebase-messaging-sw.js');
    const reg = await navigator.serviceWorker.ready;

    // ③ 기존 push 구독 완전 해제 (키 불일치로 인한 오류 방지)
    const existingSub = await reg.pushManager.getSubscription();
    if (existingSub) await existingSub.unsubscribe();

    // ④ push 구독을 직접 생성 (Uint8Array로 변환 — iOS Safari 호환)
    await reg.pushManager.subscribe({
      userVisibleOnly: true,
      applicationServerKey: urlBase64ToUint8Array(VAPID_KEY),
    });

    // ⑤ FCM 토큰 발급 (Firebase가 위에서 생성된 구독을 감지해 토큰 발급)
    const messaging = getMessaging(getApp());
