...
const LoveStats = ({ currentUser }) => {
  ...
  const [period,   setPeriod]   = useState("전체"); // "전체" | "30일" | "7일"

  useEffect(() => {
    let done = 0;
    const check = () => { done++; if (done === 5) setLoading(false); };

    const u1 = onSnapshot(query(collection(db, "diaries"),    orderBy("createdAt", "desc")), s => { setDiaries(s.docs.map(d => ({ id: d.id, ...d.data() }))); check(); });
    const u2 = onSnapshot(query(collection(db, "coupons"),    orderBy("createdAt", "desc")), s => { setCoupons(s.docs.map(d => ({ id: d.id, ...d.data() }))); check(); });
    const u3 = onSnapshot(query(collection(db, "travelPins"), orderBy("createdAt", "desc")), s => { setPins(s.docs.map(d => ({ id: d.id, ...d.data() }))); check(); });
    const u4 = onSnapshot(query(collection(db, "buckets"),    orderBy("createdAt", "desc")), s => { setBuckets(s.docs.map(d => ({ id: d.id, ...d.data() }))); check(); });
    const u5 = onSnapshot(query(collection(db, "emotions"),   orderBy("date",      "desc")), s => { setThermo(s.docs.map(d => ({ id: d.id, ...d.data() }))); check(); });
  ...
  // 좋아요 많은 일기
  const topLiked = [...filteredDiaries].sort((a, b) => (b.likes?.length || 0) - (a.likes?.length || 0))[0];
  ...
  const couponRate = filteredCoupons.length > 0
    ? Math.round((usedCoupons.length / filteredCoupons.length) * 100)
    : 0;
  ...
  const pinCatData = Object.entries(pinCatCount)
    ...
    .map(([label, value]) => {
      ...
      return { label, value, emoji: catEmojis[label] || "📍" };
    });
  ...
  // ── 버킷리스트 통계 ─────────────────────────────────────────────
  const doneBuckets = buckets.filter(b => b.done);
  const bucketRate  = buckets.length > 0 ? Math.round((doneBuckets.length / buckets.length) * 100) : 0;

  // ── 감정 온도 통계 ──────────────────────────────────────────────
  const recent7 = thermo.slice(0, 14);
  const avgTemp = recent7.length > 0
    ? Math.round(recent7.reduce((s, t) => s + (parseInt(t.temp) || 0), 0) / recent7.length)
    : 0;
  ...
  return (
    <Box sx={{ pb: 4 }}>
      ...
      <Box sx={{
        ...
        <Typography sx={{ fontSize: "0.75rem", color: B.dark + "77", mt: 0.3, fontFamily: "'Noto Sans KR',sans-serif" }}>
      ...
      <Stack spacing={2.5}>
        ...
        <Box>
          ...
          <Stack direction="row" gap={1.2}>
            <StatCard emoji="✍️" label="총 기록" value={`${filteredDiaries.length}개`}
              sub={`지수 ${jsuCount} · 현하 ${hyCount}`} color={B.accent} bgColor="#FFF5EE" />
            <StatCard emoji="🎟️" label="쿠폰 사용률" value={`${couponRate}%`}
              sub={`${usedCoupons.length}/${filteredCoupons.length}장`} color={B.pants} bgColor={B.lavender + "44"} />
          </Stack>
          <Stack direction="row" gap={1.2} mt={1.2}>
            <StatCard emoji="📍" label="방문 장소" value={`${filteredPins.length}곳`}
              sub={`지수 ${jsuPins} · 현하 ${hyPins}`} color="#3A86FF" bgColor="#F0F7FF" />
            <StatCard emoji="🪣" label="버킷리스트" value={`${bucketRate}%`}
              sub={`${doneBuckets.length}/${buckets.length}개 달성`} color={B.green} bgColor="#F0FBF6" />
          </Stack>
        </Box>
          ...
          <Stack spacing={1.5}>
            ...
            <Box>
              <Stack direction="row" justifyContent="space-between" mb={0.6}>
                <Typography sx={{ fontSize: "0.75rem", color: B.dark + "88", fontFamily: "'Noto Sans KR',sans-serif" }}>
                  사용률
                </Typography>
            ...
            <Stack direction="row" gap={1}>
              <Box sx={{ flex: 1, bgcolor: B.green + "15", borderRadius: 2, p: 1.2, textAlign: "center",
                border: `1px solid ${B.green}33` }}>
                <Typography sx={{ fontFamily: "'Jua',sans-serif", fontSize: "1.2rem", color: B.green }}>
                  {availCoupons.length}
                </Typography>
                <Typography sx={{ fontSize: "0.68rem", color: B.dark + "77", fontFamily: "'Noto Sans KR',sans-serif" }}>
                  사용 가능
                </Typography>
              </Box>
              <Box sx={{ flex: 1, bgcolor: B.pants + "15", borderRadius: 2, p: 1.2, textAlign: "center",
            ...
            {topCoupon && (
              <Box sx={{ bgcolor: B.lavender + "55", borderRadius: 2, p: 1.2,
                ...
                <Typography sx={{ fontSize: "0.72rem", color: B.dark + "88", fontFamily: "'Noto Sans KR',sans-serif" }}>
                  🏆 가장 많이 쓴 쿠폰
                </Typography>
        ...
        <Box sx={{ bgcolor: "white", borderRadius: 3, p: 2, border: "1.5px solid #3A86FF22" }}>
          ...
          {filteredPins.length === 0 ? (
            ...
            <Stack spacing={1.5}>
              <Stack direction="row" gap={1}>
                <Box sx={{ flex: 1, bgcolor: "#F0F7FF", borderRadius: 2, p: 1.2, textAlign: "center",
                  border: "1px solid #3A86FF22" }}>
                ...
                <Box sx={{ flex: 1, bgcolor: "#F0FBF6", borderRadius: 2, p: 1.2, textAlign: "center",
                  border: `1px solid ${B.green}33` }}>
                  <Typography sx={{ fontFamily: "'Jua',sans-serif", fontSize: "1.2rem", color: B.green }}>
        ...
        <Box sx={{ bgcolor: "white", borderRadius: 3, p: 2, border: `1.5px solid ${B.green}22` }}>
          ...
          <Stack spacing={1.2}>
            ...
            <Stack direction="row" gap={1}>
              ...
              <Box sx={{ flex: 1, bgcolor: B.peach, borderRadius: 2, p: 1.2, textAlign: "center",
        ...
        {/* ── 좋아요 많은 기록 ── */}
        {topLiked && topLiked.likes?.length > 0 && (
